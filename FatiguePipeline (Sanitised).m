function out = FatiguePipeline(geomCsv, telemetry, opt)
% FatiguePipeline - TBRE26 PER-MEMBER FATIGUE PIPELINE
% ------------------------------------------------------------------------
...
%   out = FatiguePipeline('SuspensionPoints(forScript).csv', tel, opt);
%
...
if nargin<1 || isempty(geomCsv)
    GEO = default_geometry();           % falls back to hard-coded points
else
...
for k = 1:Ns
    Fcp = load_transfer(Gx(k),Gy(k),Gz(k), P);   % struct .FL/.FR/.RL/.RR = [Fx Fy Fz]
    for c = 1:4
...
if ~P.quiet
fprintf('\n=== TBRe26 STEEL MEMBER fatigue (E220 tube) ===\n');
...
if ~P.quiet
fprintf('\n=== TBRe26 CLEVIS fatigue (Al-7075, no endurance limit) ===\n');
...
function Fcp = load_transfer(Gx,Gy,Gz, P)
    ...
    Fz.FL = -(P.fw*(m*Gz*g))/2 - (P.PL*P.Flift)/2 + dWx + dWy_f - dWxp;
    Fz.FR = -(P.fw*(m*Gz*g))/2 - (P.PL*P.Flift)/2 + dWx - dWy_f - dWxp;
    Fz.RL = -((1-P.fw)*(m*Gz*g))/2 - ((1-P.PL)*P.Flift)/2 - dWx + dWy_r + dWxp;
    Fz.RR = -((1-P.fw)*(m*Gz*g))/2 - ((1-P.PL)*P.Flift)/2 - dWx - dWy_r + dWxp;
